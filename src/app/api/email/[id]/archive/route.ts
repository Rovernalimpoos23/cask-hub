...
// Archives a single Microsoft Outlook message via Microsoft Graph
// (POST /me/messages/{id}/move to the well-known "archive" folder), using the
...
//
// Body: { isPresidentInbox?: boolean }
//   isPresidentInbox true  → archive in Calin's mailbox (president inbox)
//   isPresidentInbox false → archive in the signed-in user's own mailbox
//
...
// signed-in user from archiving Calin's mail by passing isPresidentInbox: true.
...
async function ensureAccessToken(
  ...
  if (needsRefresh) {
    ...
    if (!refreshRes.ok) {
      ...
      console.error('[email-archive] token refresh failed:', refreshRes.status)
      return { error: 'token_invalid', status: 401 }
    }
    ...
    if (updateErr) {
      ...
      console.error('[email-archive] token persist failed')
    }
  }
...
export async function POST(
  ...
  try {
    ...
    // ── 2. Parse the request body (isPresidentInbox is the only field) ─
    // Body is optional/empty for the own-mailbox case; tolerate a missing or
    // unparseable body by defaulting to {}.
    let body: { isPresidentInbox?: unknown }
    try {
      body = ((await request.json()) as { isPresidentInbox?: unknown }) ?? {}
    } catch {
      body = {}
    }
    ...
    if (isPresidentInbox) {
      ...
      if (callerErr) {
        console.error('[email-archive] caller lookup failed')
      ...
      if (userErr) {
        console.error('[email-archive] user lookup failed')
        return NextResponse.json({ error: 'user_lookup' }, { status: 500 })
      }
      if (!userRow) {
    ...
    if (integErr) {
      console.error('[email-archive] integration lookup failed')
    ...
    // ── 7. Move the message to the Archive folder via Graph ──────────
    // Bearer resolves /me to the correct mailbox (caller's, or Calin's above).
    // "archive" is a Graph well-known folder name — no folder ID needed.
    ...
    if (!graphRes.ok) {
      console.error('[email-archive] graph error status:', graphRes.status)
    ...
    console.error('[email-archive] error:', err instanceof Error ? err.message : 'unknown')
