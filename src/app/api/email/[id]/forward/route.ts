...
// POST /api/email/[id]/forward
// Forwards a single Microsoft Outlook message via Microsoft Graph
// (POST /me/messages/{id}/forward with an optional comment + recipients), using
// the token stored in user_integrations. Refreshes the access token automatically
// when it's expired or within 5 minutes of expiry. Auth + token pattern mirrors
// the other email routes (inlined per-route).
//
// Body: { to: string[], comment?: string, isPresidentInbox?: boolean }
//   isPresidentInbox true  → forward from Calin's mailbox (president inbox)
//   isPresidentInbox false → forward from the signed-in user's own mailbox
//
...
// and other email routes enforce. Gating the isPresidentInbox path prevents any
// signed-in user from forwarding Calin's mail by passing isPresidentInbox: true.
const ADMIN_ROLES = ['president', 'ea', 'ai_specialist']

// A recipient list is valid when it's a non-empty array of strings.
...
async function ensureAccessToken(
  ...
  if (needsRefresh) {
    ...
    if (!refreshRes.ok) {
      ...
      console.error('[email-forward] token refresh failed:', refreshRes.status)
      return { error: 'token_invalid', status: 401 }
    }
    ...
    if (updateErr) {
      ...
      console.error('[email-forward] token persist failed')
    }
  }
...
export async function POST(
  ...
  try {
    ...
    const to = body.to
    // comment is optional; coerce a non-string to '' so the Graph payload is clean.
    const comment = typeof body.comment === 'string' ? body.comment : ''
    // cc / bcc are optional; accept them only when they're valid string arrays,
    // otherwise treat as absent (→ empty recipient list in the Graph payload).
    const cc = isStringArray(body.cc) ? body.cc : undefined
    const bcc = isStringArray(body.bcc) ? body.bcc : undefined
    ...
    if (isPresidentInbox) {
      ...
      if (callerErr) {
        console.error('[email-forward] caller lookup failed')
      ...
      if (userErr) {
        console.error('[email-forward] user lookup failed')
        return NextResponse.json({ error: 'user_lookup' }, { status: 500 })
      }
      if (!userRow) {
    ...
    if (integErr) {
      console.error('[email-forward] integration lookup failed')
    ...
    // ── 7. Forward the message via Graph ─────────────────────────────
    // Bearer resolves /me to the correct mailbox (caller's, or Calin's above).
    // toRecipients is always an array, even for a single recipient.
    const graphRes = await fetch(
      `${GRAPH_BASE}/me/messages/${encodeURIComponent(messageId)}/forward`,
      {
        method: 'POST',
        headers: {
    ...
    if (!graphRes.ok) {
      console.error('[email-forward] graph error status:', graphRes.status)
    ...
    console.error('[email-forward] error:', err instanceof Error ? err.message : 'unknown')
