  ...
  try {
    ...
    for (let i = 0; i < meetings.length; i++) {
      ...
      try {
        ...
        // This meeting's chunks are embedded up front in one batched call (previously
        // one sequential Voyage request per chunk inside the loop below, multiplied by
        // every meeting in the migration — the worst offender for rate limits).
