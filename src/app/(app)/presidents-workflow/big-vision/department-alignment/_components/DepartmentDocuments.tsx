'use client'
// src/app/(app)/presidents-workflow/big-vision/department-alignment/_components/DepartmentDocuments.tsx
//
// Per-department Documents & Files block — file list + upload zone, scoped to a
// single department. Ported from the Big Vision Documents page (which uses the
// cask_vision_files table); this variant reads/writes cask_department_files and
// only ever touches rows where department = the `department` prop.
//
// Files live in the existing cask-vision-docs Supabase Storage bucket under
// department/[department]/[timestamp]-[filename].
...
export interface DepartmentDocumentsProps {
  // Department tag stored on every row, e.g. 'sales-marketing', 'finance',
  // 'operations', 'human-resources'. Used to scope reads, inserts and storage path.
  department: string
}
...
export default function DepartmentDocuments({ department }: DepartmentDocumentsProps) {
  ...
  async function handleFile(file: File | undefined) {
    ...
    try {
      const supabase = createClient()
      // Department-scoped, collision-safe path: department/[department]/[timestamp]-[filename]
      const safeName = file.name.replace(/[^a-zA-Z0-9._-]/g, '_')
      const path = `department/${department}/${Date.now()}-${safeName}`
  ...
  async function handleDelete(file: FileRow) {
    ...
    try {
      const supabase = createClient()
      const { error: deleteErr } = await supabase.from('cask_department_files').delete().eq('id', file.id)
    ...
    <>
      ...
      >
        ...
        <div style={{ fontSize: 11, color: 'var(--text3)', marginTop: 10 }}>
          Accepted: PDF, DOCX, images · Max 10MB
        </div>
