...
// Shared Compose modal (was an inline, byte-identical copy in this file and in the
// other mailbox page). RecipientInput comes from the same module because
// ForwardModal below still renders it.
import ComposeModal, { RecipientInput } from '@/components/ComposeModal'
...
export default function PresidentInboxPage() {
  ...
  // Compose prefill, set only when arrived at via a deep link carrying
  // ?prefillSubject=…&prefillBody=… (e.g. the Big Vision leader agent's "Open in
  // Compose" action). null on every normal visit, which is what keeps the manual
  // "New Message" flow identical to before: ComposeModal then receives no prefill
  // props at all and every field starts empty.
  const [composePrefill, setComposePrefill] = useState<{ subject: string; body: string } | null>(null)

  // Read ?prefillSubject=…&prefillBody=… once on mount and auto-open Compose with
  // those values seeded. Uses window.location rather than useSearchParams() so this
  // page needs no Suspense boundary (same approach as my-workspace/calendar/page.tsx).
  // Either param alone is enough — a subject-only or body-only link still opens.
  // `to` / `cc` / `bcc` are deliberately left empty, so Send stays disabled until the
  // user picks a recipient. Unlike the calendar's locked invite, nothing is locked:
  // both seeded fields remain fully editable.
  useEffect(() => {
    const q = new URLSearchParams(window.location.search)
    const prefillSubject = q.get('prefillSubject')
    const prefillBody = q.get('prefillBody')
    if (!prefillSubject && !prefillBody) return
    setComposePrefill({ subject: prefillSubject ?? '', body: prefillBody ?? '' })
    setComposeOpen(true)
  }, [])
  ...
  // list and clear the reading pane. isPresidentInbox is true here (Calin's mailbox).
  async function handleArchive() {
    ...
    try {
      const res = await fetch(`/api/email/${selected.id}/archive`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ isPresidentInbox: true }),
      })
      if (res.ok) {
    ...
    <div className="grid h-full grid-cols-[220px_300px_1fr] overflow-hidden animate-page-in">
      ...
      <div className="flex flex-col gap-4 overflow-y-auto border-r border-[var(--border)] bg-[var(--surface)] p-4">
        <div>
          ...
          <h1 className="mt-0.5 text-base font-medium text-[var(--text)]">President&apos;s Inbox</h1>
        </div>
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    {selected.attachments!.map((a, i) => (
                      ...
                      a.id ? (
                        <a
                          key={a.id ?? i}
                          href={`/api/email/download-attachment?messageId=${encodeURIComponent(selected.id)}&attachmentId=${encodeURIComponent(a.id)}&isPresidentInbox=true`}
                          target="_blank"
      ...
      {composeOpen && (
        <ComposeModal
          ...
          {...(composePrefill
            ? { prefillSubject: composePrefill.subject, prefillBody: composePrefill.body }
            : {})}
        />
      )}
