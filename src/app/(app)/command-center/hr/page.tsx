...
// ── Floating HR AI — palette + chat config ───────────────────────────
...
const AI_GREETING =
  "HR AI online. I have context on the team roster, hiring pipeline, training compliance, retention, and the HR reports. Ask about team, hiring, compliance, or what to connect next."

const QUICK_PROMPTS = ['Hiring pipeline', 'Training compliance', 'What to connect?']
...
// ── Floating HR AI button + chat drawer ──────────────────────────────

function FloatingHRAI() {
  ...
  useEffect(() => {
    async function loadHistory() {
      ...
      const { data: history } = await supabase
        ...
        .eq('page_context', '/command-center/hr')
  ...
  function saveMessage(role: string, content: string) {
    ...
    createClient()
      .from('chat_history')
      .insert({ user_email: userEmailRef.current, page_context: '/command-center/hr', role, content })
  ...
  return (
    <>
      <style>{`
        @keyframes hrSlideUp { from { opacity: 0; transform: translateY(16px); } to { opacity: 1; transform: translateY(0); } }
      `}</style>

      {/* Floating button — always visible on Human Resources */}
      <button
        onClick={() => setOpen(o => !o)}
      ...
      {open && (
        <div
          style={{
            ...
            animation: 'hrSlideUp 220ms ease',
          }}
        >
          {/* Dark header */}
          <div
          ...
          <div style={{ padding: '10px 16px 14px', borderTop: `1px solid ${D.border}`, flexShrink: 0 }}>
            ...
            >
              <textarea
                ...
                onKeyDown={onKey}
                placeholder="Ask about team, hiring, compliance..."
                rows={1}
                style={{
                  flex: 1,
                  resize: 'none',
...
export default function HRDepartmentPage() {
  return (
    <>
      ...
      {/* Floating HR AI button + chat drawer — bottom-right, this page only */}
      <FloatingHRAI />
    </>
  )
}
