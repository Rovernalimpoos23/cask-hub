...
// ── Floating Finance AI — palette + chat config ──────────────────────
...
const AI_GREETING =
  "Finance AI online. I have context on cash flow, P&L, balance sheet, AR/AP aging, and the Finance reports — fed from QuickBooks Online. Ask about cash flow, P&L, forecasts, or what to connect next."

const QUICK_PROMPTS = ['Cash flow forecast', 'P&L summary', 'What to connect?']
...
// ── Floating Finance AI button + chat drawer ─────────────────────────

function FloatingFinanceAI() {
  ...
  useEffect(() => {
    async function loadHistory() {
      ...
      const { data: history } = await supabase
        ...
        .eq('page_context', '/command-center/finance')
  ...
  function saveMessage(role: string, content: string) {
    ...
    createClient()
      .from('chat_history')
      .insert({ user_email: userEmailRef.current, page_context: '/command-center/finance', role, content })
    ...
    try {
      const res = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          ...
          pageContext: '/command-center/finance',
        }),
      })
  ...
  return (
    <>
      <style>{`
        @keyframes finSlideUp { from { opacity: 0; transform: translateY(16px); } to { opacity: 1; transform: translateY(0); } }
      `}</style>

      {/* Floating button — always visible on Finance */}
      <button
        onClick={() => setOpen(o => !o)}
      ...
      {open && (
        <div
          style={{
            ...
            animation: 'finSlideUp 220ms ease',
          }}
        >
          {/* Dark header */}
          <div
          ...
          <div style={{ padding: '10px 16px 14px', borderTop: `1px solid ${D.border}`, flexShrink: 0 }}>
            ...
            >
              <textarea
                ...
                onKeyDown={onKey}
                placeholder="Ask about cash flow, P&L, QuickBooks..."
                rows={1}
                style={{
                  flex: 1,
                  resize: 'none',
...
export default function FinanceDepartmentPage() {
  return (
    <>
      ...
      {/* Floating Finance AI button + chat drawer — bottom-right, this page only */}
      <FloatingFinanceAI />
    </>
  )
}
