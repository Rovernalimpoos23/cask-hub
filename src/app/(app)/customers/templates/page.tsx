          ...
          {phase.meetings.map((item, i) => {
            ...
            return (
              ...
              >
                ...
                {/* Teams join — all M badge meetings */}
                {['PR1m','PR3m','PR5m','PD1m','PD4m','PS2m','PS4m','PS6m','PS8m','PB2m','PB5m','CG1m','CG3m','CS2m','CR1m','CF1m','CC2m','CC4m'].includes(code ?? '') && (
                  code === 'PR1m' ? (
                    <a
...
// ── Floating Templates AI — palette + chat config ────────────────────
...
const AI_GREETING =
  "Templates AI online. I have context on the standard CASK client workflow — all 57 items across every customer journey phase, meeting agendas, and email templates. Ask about phases, templates, or emails."

const AI_QUICK_PROMPTS = ['Explain the phases', 'Email templates', 'Meeting agendas']
...
// ── Floating Templates AI button + chat drawer ───────────────────────

function FloatingTemplatesAI() {
  const [open, setOpen] = useState(false)
  ...
  useEffect(() => {
    async function loadHistory() {
      ...
      const { data: history } = await supabase
        ...
        .eq('page_context', '/customers/templates')
  ...
  function saveMessage(role: string, content: string) {
    ...
    createClient()
      .from('chat_history')
      .insert({ user_email: userEmailRef.current, page_context: '/customers/templates', role, content })
    ...
    try {
      const res = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          ...
          pageContext: '/customers/templates',
        }),
      })
  ...
  return (
    <>
      <style>{`
        @keyframes templatesSlideUp { from { opacity: 0; transform: translateY(16px); } to { opacity: 1; transform: translateY(0); } }
      `}</style>

      {/* Floating button — always visible on Client Templates */}
      <button
        onClick={() => setOpen(o => !o)}
      ...
      {open && (
        <div
          style={{
            ...
            animation: 'templatesSlideUp 220ms ease',
          }}
        >
          {/* Dark header */}
          <div
          ...
          <div style={{ padding: '10px 16px 14px', borderTop: `1px solid ${AI_D.border}`, flexShrink: 0 }}>
            ...
            >
              <textarea
                ...
                onKeyDown={onKey}
                placeholder="Ask about phases, templates, emails..."
                rows={1}
                style={{
                  flex: 1,
                  resize: 'none',
...
export default function ClientTemplatesPage() {
  ...
  return (
    <>
      ...
      {/* Floating Templates AI button + chat drawer — bottom-right, this page only */}
      <FloatingTemplatesAI />
    </>
  )
}
